...
## The same file drew two different charts

The samples are regenerated by `bootstrap.py`, and after a run that changed
nothing about them, two came back modified: the map and the star chart. The
coordinates were identical to the last digit. Only the order of the elements
had moved, and the `hash=` in the SVG header had moved with it.

Both designs keep `within = set(g.slots)` so they can ask whether somebody
is on the chart in one step, and both had gone on to *walk* it. A set of
strings comes out in an order that depends on the hash seed, and Python
picks a new seed for every process — so the order was different in every
export, and the same unchanged family exported twice gave two files that
did not match.

On the star chart that was only the order of the descent lines. On the map
it reached the geometry: the order decided which of two overlapping places
kept its name when only one would fit, and it decided where the places with
no coordinates were spaced round the edge. Two exports of one family put
Bristol in two different places.

The header comment exists to answer "has this chart changed?", and it was
answering wrongly. Worse for the thing this program is for: somebody who
exports a DXF, cuts it, finds a mistake, fixes the mistake and exports
again cannot diff the two files to see what moved.

Fixed by walking `g.slots`, which is a dict and keeps the order the grid
built it in, and leaving `within` to answer questions. The test forks:
`PYTHONHASHSEED` is fixed for the life of an interpreter, so rendering
twice in one process gives the same order both times and passes while the
bug sits there. It renders all twenty designs in two subprocesses under two
seeds and compares. Eighteen were already stable, including the flagship.
